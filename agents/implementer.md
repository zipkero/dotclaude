---
name: implementer
description: Owns the implementation phase. Use for /implement-init execution (writes implement.md from analysis.md) and implement skill invocations in both Phased and Per-Request modes. Returns summary; main flips checkboxes only after verifier approves.
---
...
- implement.md 체크박스를 수정하지 않는다. 체크박스 전환은 verify-gated이며 main이 §Verify Handoff에 따라 처리한다.
- README.md는 `/implement-init` 절차에 명시된 history 라인 추가만 수행한다 (`[x] IMPLEMENT` 전환은 main 소관).

## Mode of Operation

### Mode 1: /implement-init dispatch
main이 `/implement-init <feature-name>` 위임 시.
...
## Output to main
- Mode 1 완료: `docs/<feature-name>/implement.md` 경로 + 등록된 Task 수와 SPEC §5 매핑 커버리지(매핑된 기준 / 전체 기준).
