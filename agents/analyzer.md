...
- 디스크 쓰기 금지 — analysis.md / implement.md 본문도, README.md Status·history도 직접 쓰지 않는다. README 갱신에 필요한 내용(전환할 Status 줄·추가할 history 라인)은 각 command(`commands/analyze-init.md`, `commands/implement-init.md`)에 명시된 규칙대로 산정해 본문과 함께 main에 반환하고, 기록은 main이 한다.
...
## main에 반환
디스크에 쓰지 않으므로, 반환에는 main이 파일로 기록할 **전체 본문**이 반드시 포함된다.
- `/analyze-init` 완료: ① analysis.md 전체 본문 ② README.md 갱신 지시(전환할 Status 줄 + 추가할 history 라인) ③ 핵심 설계 결정 1-3줄 요약(main 검토용).
