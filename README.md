...
### 핵심 설계 결정
- **phase 단위 작업은 agent에 맡긴다**: 산출물을 만드는 동안 읽은 코드와 검증 내용이 main 대화에 쌓이지 않도록 떼어놓는다. analyzer 산출물 본문 자체는 제한 때문에 main이 기록하지만, 그 본문을 만들려고 입력을 읽고 설계를 따지는 일은 agent 안에서만 일어난다. 어느 작업이 어느 agent로 가는지는 CLAUDE.md §agent·skill 라우팅이 소유하고, 각 agent 정의는 아래 §agents/에 정리돼 있다.
...
각 agent는 main에서 phase 작업을 받아 처리하고 결과를 main에 돌려준다. `implementer`·`verifier`는 요약·판단만 돌려주고, `analyzer`는 파일을 쓰지 못해(report-file 제한) 산출물 본문 전체를 돌려주며 기록은 main이 한다.

- `analyzer` — `/analyze-init`·`/implement-init` 실행. 계획 산출물(`analysis.md`, `implement.md`) 본문을 만들어 main에 돌려주며, 파일 기록은 main이 한다(subagent report-file 제한). 코드는 고치지 않는다.
...
- `verifier` — `verify` skill 호출 (Phased / Per-Request 모두). 판단만 돌려주며, 어떤 문서·체크박스·코드도 고치지 않는다 (뒤이은 전환은 §verify 후처리 소관).
...
- `spec-init.md` — `spec.md`를 쓰고 feature `README.md`를 초기화한다 (`/spec-init <feature-name>`). `<feature-dir>` 전체 형식(`<yyyyMMdd>-<nnn>-<feature-name>`)을 자동으로 만든다 — 자세한 룰은 `commands/spec-init.md` §산출 경로에 있다.
...
- `cross-analyze.md` — 같은 분석 질문을 N개 에이전트에 같은 프롬프트로 따로 분석시키고 main이 교차검증해 합의·불일치를 보고한다 (`/cross-analyze [N] <질문>`). 읽기 전용이다.
...
- `analyze` — 독립 디버깅·코드 이해 도구. 파일을 쓰지 않고 대화로만 출력한다.
- `implement` — Phased에서는 `implement.md`의 다음 Task를 실행하고, Per-Request에서는 산출물 없이 변경을 한다. 다음 `verify` 호출이 분명한 변경 범위를 가질 수 있도록 **Files touched** 목록을 함께 출력한다.
