...
3. `$ARGUMENTS`가 비어 있으면 다음과 같이 한다.
   - 활성 `features/<feature-dir>/` 범위가 있으면 (1)처럼 읽는다. 활성 범위의 뜻은 `implement` skill §컨텍스트 로딩과 같되, "implement 뜻"을 "분석 뜻"으로 읽는다.
   - 그 외에는 대화 맥락으로 분석하며, 맥락에 충분한 단서(에러 메시지, 파일 경로, 증상)가 있을 때 가정을 밝히고 진행한다.

## 출력 구조
필수:
1. Summary — 1-2 문장 결론. 조사 결과 할 일이 없을 때는 `No issues found`도 유효한 결론이며, 무엇을 조사했고 왜 손댈 필요가 없는지를 함께 적는다. 분석을 정당화하려고 걱정거리를 지어내지 않는다.
2. Execution flow / data flow / root cause — 핵심 분석.

상황별 (해당할 때만 넣는다):
3. Purpose or problem — 요청에 "왜"가 들어 있을 때.
4. Key observations — 곁가지 발견이 있을 때.
5. Recommendation — 사용자 요청에 답하는 데 필요할 때만 둔다. 보안·테스트·로깅 강화 같은 요청과 상관없는 개선안은 이 항목에 끌어오지 않는다.
6. 다음 단계 분류 — 사용자의 구현 뜻이 보이고 분석 결과가 구현 범위에 영향을 줄 때만 둔다. `Phased 대상`(요구사항 확정·설계 판단·여러 번의 검증 필요) 또는 `Per-Request 가능`(작고 분명하며 되돌릴 수 있음)과 한 줄 사유를 적는다.
7. Blocker — 사유를 적고 멈춘다. 세 가지다.
   ...
   모든 Blocker는 다음을 담는다.
   - Evidence: 무엇을 조사했고(파일·로그·에러) 무엇을 찾았는지.
   - Unblock requires: 막힌 것을 푸는 구체적인 정보·결정·변경.

## 지침
- 추론·구조·원인에 집중한다.
- 추상적인 설명이 아니라 실제 동작에 근거해 설명한다.
- 상태 변화·실패 지점은 관련 있을 때만 넣는다.
- Blocker는 근거를 갖고서만 선언한다. 선언 전에 관련 소스(파일·로그·에러 흐름)를 읽으며, 가정만으로 선언하지 않는다.
- 모든 Blocker는 푸는 방법을 담는다 — 어떤 정보·결정·변경이 막힌 것을 푸는지.
- 일반 점검 항목(보안·성능·규정·SLO·계약)을 대상에 그냥 갖다 붙이지 않는다. 각 항목은 보고 전에 이 프로젝트의 코드·spec에서 확인해야 하며, 확인 안 된 일반론은 출력에 넣지 않는다.
