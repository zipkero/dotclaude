...
## 역할
verify는 **Task-level 판단 도구**다. `implement` turn 직후에 실행되어 단 하나의 질문에 답한다 — 방금 만든 Task가 DoD를 만족했는가? feature-level 검증 패스는 두지 않는다. 체크박스 전환과 자동 재시도 정책은 §verify 후처리에 둔다.
...
근거 기준:
- 최소 evidence는 code diff이며, 권장은 diff + 테스트 결과다.
...
- 변경 범위에 대한 테스트가 존재하지만 실행되지 않았다면 한계로 기록한다.
