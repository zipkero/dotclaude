...
- 내부 계산·조건·변환만 고쳤고 맞게 고쳤는지가 변경 내용에 그대로 보이면, 변경 내용만으로 판정해도 된다.
...
- 첫째 기준은 대상 Task의 `목적`이 적은 동작이 성립하는지다. 검증 조건 통과로 대신하지 않고 `목적` 문장 자체를 기준으로 보며,
  spec.md·design.md가 그 경우를 다루지 않는 것은 성립의 근거가 아니다.
- 둘째 기준은 대상 Task의 검증 조건이다.
- 셋째 기준은 매핑된 `SPEC §5.N`과 spec.md의 제약·제외 범위를 어기지 않는지다. 단, 이번 approve로 완료되는 `SPEC §5.N`은
  어기지 않음이 아니라 성립 확인까지 요구한다(§완료되는 요구사항 판정).
- 넷째 기준은 design.md의 설계 결정을 벗어나지 않는지다.
...
1. Phased mode — 들어가는 조건은 `implement` skill §컨텍스트 로딩과 같되, "implement 뜻"을 "검증 뜻"으로 읽는다.
   ...
   - 검증할 변경 범위는 기본적으로 아직 commit되지 않은 working tree 변경분이다. 변경이 이미 commit된
     ...
     reject 뒤 같은 Task를 다시 verify할 때의 변경 범위는 reject 사유를 고친 변경분이며, 앞 판정에서 성립한 항목 중
     그 변경분에 든 파일을 근거로 삼지 않은 것만 유지한다.
...
- 위임 프롬프트에 `<feature-dir>`, 대상 Task의 `task-<nnn>` 제목, 재검증 모드인지 여부, 검증할 변경 범위를 적는다.
  직전 `implement`가 비고·한계에 남긴 내용이 있으면 그대로 옮긴다.
  spec.md·design.md·implement.md의 기준 본문은 옮겨 적거나 요약하지 않는다.
...
- 하나라도 성립하지 않으면 Task 검증 조건을 채웠는지와 상관없이 판정은 `rejected`다. 분류는
  ...
  단, 성립하지 않는 문장을 `목적`·검증 조건에 담은 Task가 대상 Task도 앞선 `[x]` Task도 아니면 코드가 아니라 매핑의 흠이므로
  reject 사유로 삼지 않고, §출력 구조 4번에 그 문장을 불성립으로 적으면서 사유를 매핑 누락으로, 담은 Task가 있으면 그 `task-<nnn>`을 밝힌다.
