...
- 2차 기준은 매핑된 `SPEC §5.N`과 spec.md의 제약·제외 범위를 위반하지 않는지다. 단, 이번 approve로 완료되는 `SPEC §5.N`은
  위반 없음이 아니라 성립 확인까지 요구한다(§완료되는 요구사항 판정).
- 3차 기준은 analysis.md의 설계 결정을 벗어나지 않는지다.

## 컨텍스트 로딩
1. Phased mode — 진입 조건은 `implement` skill §컨텍스트 로딩과 동일하다.
   - spec.md의 완료 조건과 제약·제외 범위를 읽는다 (요구사항 레벨 기준).
