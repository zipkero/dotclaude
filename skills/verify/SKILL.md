...
대상 Task를 가려내기 모호하면(여러 개가 기다리거나 직전 implement 대상이 하나로 잡히지 않는 경우), 판단 전에 후보와 사유를 묶어 main에 돌려준다(CLAUDE.md §agent·skill 라우팅).
...
2. Target Task: implement.md Task 제목(Phased) 또는 사용자가 말한 변경(Per-Request)을 인용한다.
3. Validation — 이번 Task 판단에 실제로 영향을 준 항목만 적는다. 항목을 채우려고 상관없는 spec.md §5나 다른 모듈을 근거로 끌어오지 않는다.
   ...
   - 범위·동작 정확성
   - Evidence (변경 내용, 테스트 결과, 또는 밝힌 한계)
4. Completed requirements — 완료되는 `SPEC §5.N`이 있으면 각각 성립/불성립을 근거와 함께 적고,
