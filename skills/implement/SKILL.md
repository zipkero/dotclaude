...
## 주석 작성 기준
- 이름만으로 의도가 드러나면 달지 않는다. 메서드·타입에는 책임 한 줄만, 내부 구현 절차는 쓰지 않는다.
- 코드만으로 드러나지 않는 도메인 제약·순서 불변식·바꾸면 깨지는 선택은 인라인으로 남긴다.
- 변경 경위(변경 전·후 서술, task·Decision 식별자)는 넣지 않는다. 패키지 선언에도 달지 않는다.
- 구조·형식은 해당 언어 컨벤션, 주석 언어는 파일 컨벤션을 따른다. 없으면 한국어.
- 로직을 수정하면 관련 주석도 같은 turn에 갱신한다. 사라진 의도를 가리키는 주석은 남기지 않는다.
