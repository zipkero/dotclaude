...
## 주석 작성 기준
- 이름만으로 의도가 드러나면 달지 않는다. 메서드·타입에는 호출자가 알아야 하는 책임만 남긴다.
- 코드만으로 드러나지 않는 도메인 제약·순서 불변식·바꾸면 깨지는 선택은 인라인으로 남긴다.
- 변경 경위·구현 일정 참조(전후 서술, 식별자 등)는 어디에도 넣지 않는다.
- 구조·형식은 해당 언어 컨벤션을 따른다. 주석 언어는 `CLAUDE.md` §언어를 따른다.
- 로직을 수정하면 관련 주석도 같은 turn에 갱신하거나 지운다. 사라진 의도를 가리키는 주석은 남기지 않는다.
