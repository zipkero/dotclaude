...
## 주석
- 코드만 봐서는 알 수 없는 이유·제약·늘 성립해야 하는 조건만 남긴다.
- 코드가 하는 일을 옮겨 적지 않는다. 이름·구조·타입으로 뜻이 드러나는 자리에는 쓰지 않는다.
- 다른 문서·파일·API 경로를 가리켜 설명하지 않는다.
- 작업하면서 알게 된 것은 옮기지 않는다 — `SPEC §5.N`·`ANALYSIS §X.Y`·`task-<nnn>` 같은 문서 위치,
  결정 번호, 작업 단계, 왜 이렇게 바뀌었는지, 이 코드 전후에 무엇이 일어나는지.
