...
구현 도중에는 Phased를 권하지 않는다. 권하는 시점은 요청을 분류할 때뿐이다(CLAUDE.md §phase 제어).
...
코드를 쓰지 말고 필요한 결정을 `blocked`로 보고한다. 부분 산출물을 임시로 저장하지 않으며, 결정이 난 뒤 작업을 다시 시작한다.
...
- Phased mode에서 접근 이탈이 단순 구현 상세 차이로 보고되고 design.md·목적·검증 조건·참조를 바꿀 필요가 없는 경우에만 main이 같은 응답에서
  그 Task의 접근 필드를 실제 구현 방식으로 고치고, 그렇지 않으면 문서를 고치지 않고 design.md 또는 implement.md 재작성을 사용자에게 보고한다.
...
주석 설명을 `SPEC §5.N`, `DESIGN §X.Y`, `task-<nnn>` 같은 문서 위치로 미루지 않는다.
