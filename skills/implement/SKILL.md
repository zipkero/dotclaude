...
## 주석 작성 기준
- 이름 있는 단위(특히 메소드·함수)에는 그것이 왜 존재하고 어떤 역할을 맡는지를 짧게 남기는 것을 기본으로 한다.
- 이름과 매개변수만으로 역할이 자명한 단순 유틸·얇은 위임에는 생략한다.
- 코드로 드러나지 않는 제약, 외부 시스템의 의외의 동작, 예외적 변환, 바꾸면 깨지는 선택은 역할 설명과 별개로 해당 코드 바로 옆에 짧게 남긴다.
- 공개 경계에는 호출자가 알아야 하는 역할과 계약을 짧게 남긴다.
- 변경 경위·구현 순서·계획 식별자 같은 작업 과정 메타는 주석으로 남기지 않는다.
