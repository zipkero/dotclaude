...
코드 단위 의도는 코드 옆 주석으로 남긴다. 흐름·존재 이유의 메타 해설은 이 커맨드의 범위가 아니라 `/explain-flow`의 영역이다.

### 1. 타입/메서드/함수 역할 주석
- 주요 클래스·구조체·인터페이스·메서드·함수에는 역할 주석을 작성한다.
- 주석 형식(docstring·JSDoc·Godoc 등)은 해당 파일·패키지의 기존 컨벤션을 따른다. 새 형식을 도입하지 않는다.
- 주석 언어도 해당 파일·패키지의 기존 컨벤션을 따른다. 컨벤션이 없으면 한국어로 쓴다. 공개 API의 docstring·Godoc은 영어로 쓴다.
- 역할 주석은 내부 구현 절차가 아니라 "이 코드가 전체 흐름에서 맡는 책임"을 설명한다.
- 입력·출력·호출 시점·소유한 책임이 코드 이름만으로 충분히 드러나지 않으면 주석을 남긴다.
- 단순 getter/setter, 테스트 함수·헬퍼, 이름만으로 역할이 분명한 작은 함수에는 억지로 주석을 달지 않는다.

### 2. 핵심 로직 내부 주석
- 핵심 로직 내부에는 필요한 단위로 블록 주석을 작성한다.
- 내부 주석은 한 줄마다 설명하지 말고, 분기·변환·상태 변경·외부 연동처럼 흐름을 놓치기 쉬운 지점에 둔다.
- 주석은 "이 블록이 무엇을 처리하는지"를 먼저 설명하고, 코드만으로 의도가 드러나지 않으면 "왜 이렇게 처리하는지"까지 설명한다.
- 도메인 규칙, 예외 처리 기준, 순서가 중요한 처리, 바꾸면 깨지는 의도적 선택은 주석 후보로 본다.
- 작업 컨텍스트(task-<nnn>, 이슈 번호, "방금 추가됨", 작성자명 등)는 주석에 남기지 않는다. 그건 커밋 메시지·PR에 둔다.
- 자명한 대입, 단순 반복, 함수명 그대로의 호출에는 주석을 달지 않는다.

### 3. 주석 갱신 책임
- 로직을 수정하면 같은 turn 안에서 관련 주석도 함께 갱신한다. 코드와 주석의 불일치를 다음 작업으로 미루지 않는다.
