...
코드 단위 의도는 코드 옆 주석으로 남긴다. 흐름·존재 이유의 메타 해설은 이 skill의 범위가 아니라 `/flow`의 영역이다.

다음은 모든 주석에 공통으로 적용되는 원칙이다.
- 주석은 코드와 수명을 같이할 의도만 담는다.
- 이름만으로 의도가 드러나는 코드에는 주석을 달지 않는다.
- 구현 과정의 변경 경위(변경 전·후 서술, task·Decision 식별자, changelog 내용)는 주석에 넣지 않는다.

### 1. 타입/메서드/함수 역할 주석
- 이름만으로 입력·출력·호출 시점·소유한 책임이 드러나지 않는 타입·인터페이스·메서드·함수에는 역할 주석을 단다.
- 역할 주석은 내부 구현 절차가 아니라 이 코드가 전체 구조에서 맡는 책임을 설명한다.
- 주석 형식(docstring·JSDoc·Godoc 등)과 언어는 해당 파일·패키지의 기존 컨벤션을 따른다. 컨벤션이 없으면 한국어로 쓰되, 공개 API의 docstring·Godoc은 영어로 쓴다.

### 2. 패키지·네임스페이스 doc 주석
- 패키지·네임스페이스 선언에 doc 주석을 달지 않는다. 묶음의 의미는 이름과 그 안의 타입이 표현한다.

### 3. 핵심 로직 내부 주석
- 블록 주석은 분기·변환·상태 변경·외부 연동처럼 흐름이 갈리는 지점에 둔다.
- 블록 주석은 "이 블록이 무엇을 처리하는지"를 먼저 설명하고, 코드만으로 의도가 드러나지 않으면 "왜 이렇게 처리하는지"까지 설명한다.
- 도메인 규칙, 예외 처리 기준, 순서가 중요한 처리, 바꾸면 깨지는 의도적 선택은 주석으로 남길 가치가 있다.
- 코드를 재진술하는 인라인 라벨(변수명·타입명·필드명 반복 등)은 달지 않는다.
- 위 기준 중 어느 것도 해당하지 않으면 주석을 달지 않는다.

### 4. 주석 갱신 책임
- 로직을 수정하면 같은 turn 안에서 관련 주석도 함께 갱신한다.
- 주석만 수정해 의미가 보존되는 경우와, 주석을 통째로 들어내야 하는 경우(원래 설명하던 분기·조건이 사라진 경우)를 구분한다. 사라진 의도를 가리키는 주석은 남기지 않는다.
