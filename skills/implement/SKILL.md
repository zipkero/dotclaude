...
- 새 인터페이스·추상화·public API·경계
- 새 의존성·설정 항목
- 요청한 변경이 앞뒤가 맞는 데 필요한 범위를 넘는 인접 코드 리팩터링

이 중 어느 것이라도 요청을 채우는 데 필요해 보이면, 코드를 쓰지 말고 사용자에게 먼저 의견을 묻는다. 사용자가 명시적으로 허락한 경우에만 진행한다.
조용히 범위를 넓히지 않는다. Phased 권장은 요청을 분류하는 시점에만 하며(CLAUDE.md §phase 제어), Per-Request mode로 들어온 뒤에는 `/spec-init`로
옮기라고 안내하지 않는다.
...
4. 고친 파일 — 고친 경로 목록. 다음 `verify` 호출의 변경 범위로 쓰이며, 변경이 verify 전에 commit되었을 때 특히 쓸모 있다.
