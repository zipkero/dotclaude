...
- "현재 → 수정안" 또는 "변경 전 → 변경 후"를 보여줄 때는 `diff` 코드 블록 안에서 `-`/`+` 라인으로 표현한다. 평문 bullet 두 개로 나열하지 않는다. 변경 제안뿐 아니라 변경을 설명·회고하는 응답에도 적용한다. 단순 사실 진술이나 구조 설명처럼 before/after가 없는 응답에는 적용하지 않는다.
...
- 읽기 전용 탐색이 10+ 파일에 걸치거나 전역 키워드 조사가 필요한 경우 `Explore` subagent로 main 컨텍스트를 보호한다. 호출 시 `model: sonnet`을 명시한다.
