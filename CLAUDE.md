...
- `.claude/` 파일에서 본문(룰·설명)은 한국어로 작성하되, frontmatter의 `name`/`description`과 시스템 식별자(`Phased`, `Per-Request`, `[ ]`/`[x]`, `approved`/`rejected`, `$ARGUMENTS`, `spec.md`, `task-<nnn>` 등)는 영어로 둔다.
...
- 부연은 외부 독자가 맥락 없이 읽어도 따라올 수 있는 완결된 문장으로 풀어 쓴다.
...
## Code
- 분석·검토·설명·비교·제안 요청은 구현 요청으로 해석하지 않는다. 구현이 필요해 보여도 코드를 쓰기 전에 범위와 다음 단계를 먼저 보고한다.
...
### Feature README 소유
- slash command·agent의 자동 README 갱신 대상은 feature별 `docs/<feature-dir>/README.md`로 한정한다. repo-root `/README.md`(이 설정 자체의 설명)는 설정 구조 변경에 따른 의도적 동기화가 필요할 때 사용자 지시에 따라서만 수정한다.
- README Status 세 라인은 spec-init이 README 신규 생성 시 한 번에 박는다 — `[x] SPEC`, `[ ] ANALYSIS`, `[ ] IMPLEMENT`. 이후 라인 전환 책임은 다음과 같다.
  ...
  각 command의 자세한 전환 룰은 해당 command 파일에 둔다. `[x] IMPLEMENT` 전환과 rejected 재검증 시 역전환(`[x] → [ ]`)은 §verify 후처리가 단독으로 정의한다.
...
- slash command의 덮어쓰기 의미는 각 command 파일의 덮어쓰기 규칙 섹션에 둔다.
