...
## Language & Tone
- 응답은 한국어로, 격식 있는 캐주얼 톤을 쓴다. 직접적이고 사실 기반으로 답한다.
...
- 사용자의 방향이 근거상 부적절해 보이면, 진행하기 전에 이견과 근거를 먼저 제시한다. 의심스러운 지시를 묵묵히 따르지 않는다.
...
### Defaults
- 사용자 turn당 사용자 응답은 1개로 한정하며, 내부 orchestration은 예외로 둔다.
- 단계별로 진행하는 것은 사용자가 명시적으로 요청했거나 작업이 상태 변경이 있거나 되돌릴 수 없을 때로 한정한다.

### Flows
flow는 두 가지다. Phased는 산출물(spec → analysis → implement)을 작업 기록으로 보존하고, Per-Request는 영속 문서 없이 바로 실행한다. 핸드오프, 후속 참조, 간단치 않은 범위처럼 지속적인 기록이 가치 있을 때는 Phased를, 그 외에는 Per-Request를 택한다.

- **Phased** (사용자 주도): `prompt → /spec-init → /analyze-init → /implement-init → implement → verify`. 각 phase가 작성한 문서를 다음 phase가 소비한다. slash command는 사용자가 직접 호출하며 자동으로 이어지지 않는다. `implement`와 `verify`는 자연어 트리거이므로, 사용자가 phase 진행 시점을 통제한다.
...
- 읽기 전용 탐색에서 여러 파일에 걸친 대량 grep/Read가 필요한 경우(예: 10+ 파일에 걸친 흐름 추적, 코드베이스 전역 키워드 조사)에는 `Explore` subagent를 쓸 수 있다. Explore가 main에 요약을 반환하므로 원본 결과가 main context를 차지하지 않는다. 단일 파일 조회나 직접 Read/Grep으로 더 싸게 끝나는 조회에는 쓰지 않는다.
...
- **Approved**: main이 대상 implement.md 체크박스를 `[ ]`에서 `[x]`로 바꾸고 다른 파일은 건드리지 않는다. 결과는 사용자에게 알린다.
- **Rejected**:
  - 대상 체크박스가 `[ ]`였다면(implement 직후의 일반 케이스) 그대로 둔다.
  - 이미 `[x]`였던 Task를 재검증하다가 reject되면 main이 `[x]`를 `[ ]`로 되돌린다.
...
- 각 command의 Status 전환 룰은 해당 command 파일(`commands/spec-init.md`, `commands/analyze-init.md`, `commands/implement-init.md`)에 둔다.
