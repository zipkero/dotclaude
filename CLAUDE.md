...
### Orchestration Rules
- phase 단위 작업은 해당 agent에 위임한다. 각 agent는 그 phase의 잡음(코드 탐색·산출물 작성·테스트 실행)을 흡수하고 main에는 산출 요약만 돌려준다. 절차의 권위는 해당 command·skill 파일에 두고, agent 본문은 그 파일들을 참조한다.
  - main: `/spec-init` 실행 (요구사항 ambiguity가 사용자 대화로 해소되는 phase는 위임하지 않는다)
  - `analyzer`: `/analyze-init` 실행, `analyze` skill 호출
  - `implementer`: `/implement-init` 실행, `implement` skill 호출 (Phased / Per-Request 모두)
  - `verifier`: `verify` skill 호출
- agent 위임 시 main은 작업 의도와 인자(`feature-name` 등)만 전달하고 command·skill 본문을 main 컨텍스트에 펼치지 않는다.
- agent가 작업 시작 전 ambiguity나 사용자 결정이 필요하면 (예: `/implement-init`의 미매핑 SPEC §5 처리, `implement`의 Non-expansion baseline 위반, `analyze`의 `needs input` Blocker) 코드·문서를 건드리기 전에 질문 항목을 묶어 main에 반환한다. 처리 분기:
