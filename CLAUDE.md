...
- **Per-Request**: `prompt → implement → verify`. slash command가 없고 활성 `docs/<feature>/` scope도 없는 자연어 prompt에서 진입한다. 두 mode 모두 verify는 사용자 명시 호출이 필요하며, 자동으로 이어지지 않는다 (§Verify Handoff).
