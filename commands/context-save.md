...
- feature 산출물이나 지정된 진행 추적자가 있으면 Task 상태, 요구사항과 구현 계획은 해당 문서를 기준으로 유지한다.
- `CONTEXT.md`에는 작업을 중단시킨 설계 변경, 현재 논점, 이어서 볼 파일과 다음 작업만 기록한다.
- feature 산출물도 진행 추적자도 없으면 사용자 요청 범위, 변경한 파일과 실행한 검증 결과를 기준으로 기록한다.
...
- `/context-save`는 `CONTEXT.md`만 변경한다.
  ...
  원본 갱신은 `rules/feature-docs.md`가 정한 소유 주체가 한다.

## 저장 절차

1. 프로젝트 루트의 기존 `CONTEXT.md`가 있으면 전체를 읽는다.
2. 현재 대화에서 사용자가 확정한 목표와 결정, 아직 정해지지 않은 판단, 다음 작업을 추린다.
3. 관련 feature 산출물과 프로젝트 문서를 읽어 실제 반영 여부와 정확한 문서 경로를 확인한다.
...
- `현재 상태`는 완료된 범위와 멈춘 지점을 적고 세부 작업 이력을 나열하지 않는다.
  아래를 각각 한 줄로 덧붙이고, 해당 없으면 그 줄을 생략한다.
  - 마지막으로 실행한 검증과 그 결과. 실행한 적이 없으면 그렇게 적는다.
  - 아직 해소되지 않은 멈춤 원인과 그 해소 조건.
  - 대상이 Git 저장소이면 현재 branch와 기준 HEAD.
- `현재 작업 문서`에는 활성 feature와 Task 또는 지정된 진행 추적자가 있으면 그 문서와 현재 항목을 링크한다.
  추적자를 링크할 때는 그것이 feature 산출물이 아니라 진행 추적자라는 것을 함께 적는다.
...
- `문서 반영 필요`에는 확정됐지만 원본 또는 feature 산출물에 아직 반영되지 않은 내용만 적는다.
- 적을 내용이 없는 항목은 `없음`으로 적는다.
