...
### 재시도 구현의 파급 점검
점검 기준은 `skills/implement/SKILL.md` §재작업 시 파급 점검이 소유한다. 재시도로 돌아갈 때 그 점검이 필요한 재작업임을
reject 사유와 함께 `implement` 입력에 밝힌다. 점검 결과 설계 변경이 필요하다고 보고되면 §정지 조건 1로 간다.
앞선 `[x]` Task의 코드에 걸치는 수정도 같은 Task의 재작업으로 본다(§verify 후처리와 같은 범위).
