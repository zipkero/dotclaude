...
- `수정 소유 단계`가 `implement`가 아닌 문제는 재시도 대상이 아니다 — §정지 조건 1로 간다. `reject 분류`와 `수정 소유 단계`가
  어긋나 보이는 조합이 와도 재시도·정지 판정은 `수정 소유 단계`만 따른다(`skills/verify/SKILL.md` §reject 분류가 분류를
  참고값으로만 규정한다).
...
4. 다음 행동 — 정지 조건 1이면 `수정 소유 단계`가 가리키는 문서와 섹션을 짚는다. 여러 문서를 고쳐야 하면 수정 순서는
   CLAUDE.md §문서 구조를 따른다.
