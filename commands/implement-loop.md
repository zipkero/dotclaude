...
## 재시도
- 지적받은 문제가 **구현 수정만으로 풀리면** 같은 Task로 3번으로 돌아간다. 체크박스는 `[ ]`로 둔다.
- 재시도할 때 verify가 낸 reject 사유·근거를 다음 `implement` 입력에 그대로 넘긴다. 같은 지적을 다시 받지 않게 하는 것이 목적이다.
- 한도는 재시도 2회다 (한 Task당 최대 3번 구현). 소진하면 정지한다.
- spec.md나 analysis.md를 고쳐야 풀리는 문제는 재시도 대상이 아니다 — §정지 조건 1로 간다.

### 재시도 구현의 파급 점검
재시도는 지적받은 자리만 고치고 끝내지 않는다. 고친 뒤 다음을 확인하고 결과를 보고에 담는다.

- 고친 함수·타입·필드·설정을 쓰는 다른 호출부가 같은 가정 위에 있었는지
- 같은 Task 안에서 앞선 시도에 이미 성립했던 동작이 그대로인지
- 이미 `[x]`인 Task가 만든 동작이 깨지지 않았는지 — 공유 코드·공개 시그니처·저장 형식·설정을 건드렸으면 특히 본다
- 변경 범위에 걸리는 기존 테스트·빌드가 여전히 통과하는지

영향이 발견되면 같은 재시도 안에서 함께 고친다 (§verify 후처리가 정한 재작업 범위와 같다 — 앞선 `[x]` Task의 코드에 걸쳐도 같은 Task의 재작업으로 본다).
다만 고치는 데 설계 변경이 필요하면 고치지 말고 §정지 조건 1로 간다. 확인하지 못한 항목은 한계로 밝힌다.
파급을 미리 막겠다고 요청 범위 밖을 리팩터링하지 않는다.

## 자동 진행 제외
대상 Task의 검증 조건 `확인` 필드가 실행 가능한 근거(테스트·빌드·lint·명령 출력)를 가리키지 않고 수동 확인에만 기대면,
그 Task는 자동으로 진행하지 않고 멈춰 사용자에게 올린다. 루프가 자기 눈으로 자기 작업을 승인하는 구간을 만들지 않기 위한 것이다.

## 정지 조건
아래 중 하나를 만나면 루프를 멈추고 §정지·완료 보고를 낸다. 남은 Task는 건드리지 않는다.

1. **spec.md 또는 analysis.md를 고쳐야 하는 경우** — 형태를 가리지 않는다.
   - verify가 `design/scope`로 rejected 한 경우
...
4. 대상 Task가 외부에서 관찰할 수 있는 동작 하나를 완성하지 못해 implement가 재분할을 요구한 경우
...
- **spec.md와 analysis.md를 고치지 않는다.** 판정 기준을 스스로 바꾸는 일이기 때문이다. 고쳐야 하는 상황은 정지 조건 1로 올린다.
  - 예외는 하나다 — `skills/implement/SKILL.md` §완료가 허용하는 접근 필드 정정(단순 구현 상세 차이). implement.md의 접근 필드만 바뀌고 판정 기준은 그대로다.
...
4. 다음 행동 — 정지 조건 1이면 어느 문서 어느 섹션을 봐야 하는지 짚는다. 수정 순서는 CLAUDE.md §문서 구조를 따른다
   (spec.md를 먼저 고치고 영향받는 analysis.md → implement.md 부분에만 반영).
