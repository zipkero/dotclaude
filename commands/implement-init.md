...
### Task 형식 (한국어 산출물 템플릿)
각 Task는 한 verify 사이클로 평가 가능한 단위다 — **외부 관찰 가능한 동작 하나와 그 회귀 보호**가 기준이다.

분리 기준:
- 검증 기준이 갈리는 지점에서 분리한다.
- 같은 외부 동작에 기여하는 구현은 한 Task에 둔다.
...
## 테스트 Task 포함 기준
analysis.md가 의미 있는 회귀 위험(상태 변화, 외부 I/O, 동시성, 신규 경계)을 시사할 때만 테스트를 추가한다. 테스트 코드 작성 범위·예외는 `skills/implement/SKILL.md` §테스트 코드 작성이 소유한다.

회귀 테스트는 구현 Task의 `확인` 필드 안에 둔다. 별도 테스트 Task는 테스트가 여러 구현에 걸치거나 독립적인 검증 산출물(예: 여러 흐름을 묶는 e2e)일 때만 둔다.
...
- 접근 필드에 파일 배치·분리를 지정하지 않는다 — 구현 시점의 디렉토리 컨벤션 소관 (`skills/implement/SKILL.md` §지침).
