...
Task ID 규칙:
- 모든 Task에 전역 일련번호 prefix `T<n>`을 붙인다 (`T1`, `T2`, ...). 그룹(`## Section:`)이 있어도 번호는 리셋하지 않고 문서 전체에서 연속한다.
- 번호는 §Ordering의 의존성 기준 정렬 순서를 그대로 따른다 — 시간 순서가 아니라 "다음을 가능하게 하는 선행 조건이 먼저"라는 위치 의미를 담는다.
- ID는 식별·언급용이며, Task 사이 의존성을 별도 필드로 명시하지 않는다 (위치가 곧 순서).
...
구체 예시:
```
- [ ] T3: OrderService.fetchOrders 구현
...
- Task ID(`T<n>`)는 이 정렬 순서를 그대로 반영한다. 순서가 바뀌면 ID도 다시 매긴다.
