...
- 결과는 대화 응답에만 남는다. agent도 main도 **파일을 만들거나 수정하지 않는다** (읽기 전용 분석).
...
1. **분석 브리프 1개 작성** — main이 대상 질문을 다음을 포함한 단일 프롬프트로 정리한다. 이 프롬프트는 **모든 agent에게 토씨까지 동일하게** 전달한다 (결론이 모이는지 보는 게 목적이므로 각도를 다르게 주지 않는다).
...
4. **교차검증** — 반환된 N개 보고를 §교차검증 규칙으로 대조해 하나로 합친다. 필요하면 main이 결정적 근거 1~2곳만 직접 열어 확인한다(전면 재분석 금지).
...
## 금지
- 파일 생성·수정·삭제. (읽기 전용)
- agent마다 다른 각도를 주는 것. 이 command는 결론이 모이는지 보는 게 목적이라 프롬프트를 똑같이 둔다. (각도를 나누고 싶으면 이 command가 아니라 개별 지시로 처리)
