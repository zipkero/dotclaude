...
- `저장:` 시각은 인수인계가 얼마나 오래됐는지 알리는 값이며, 충돌 판정 근거로 쓰지 않는다.
  이 command는 어느 쪽이 최신인지 스스로 판정하지 않는다.
- 대상 프로젝트 루트는 복원을 이어가기 전에 `commands/project-init.md` §대상 프로젝트 루트를 따라 확인한다.

## 복원 절차

1. 프로젝트 루트의 `CONTEXT.md`를 전체 읽는다.
2. 저장된 맥락과 원본 문서에서 확인되지 않는 내용은 어떤 경우에도 추정으로 채우지 않는다.
   파일이 없으면 저장된 맥락이 없다고 알린 뒤, 확인된 루트의 프로젝트 문서와 `features/` 아래 feature README
...
5. `CONTEXT.md`와 원본이 충돌하면 항목이 `문서 반영 필요`에 있는지와 무관하게 양쪽 내용을 함께 보고하고,
   어느 쪽이 현재 결정인지 확정하지 않는다. 저장된 맥락이 오래돼 보인다는 이유로 한쪽을 버리지 않는다.
...
- `CONTEXT.md`에 없는 새 설계 결정은 확정하지도, 추정·제안·판단으로 표시해 보고에 넣지도 않는다.
  복원 보고에는 추정을 담지 않으므로 `CLAUDE.md` §응답의 근거·추정 분리로 나눠 적을 추정 항목이 없다.
