...
- feature 산출물이나 지정된 진행 추적자가 있으면 Task 상태, 요구사항과 구현 계획은 해당 문서를 기준으로 복원한다.
- feature 산출물도 진행 추적자도 없으면 저장된 요청 범위와 현재 작업 트리를 기준으로 복원한다.
...
2. 파일이 없으면 저장된 맥락이 없다고 알린 뒤, 프로젝트 루트의 문서와 `features/` 아래 feature README
...
5. 참조 파일이 없거나 `CONTEXT.md`와 원본·feature 산출물이 충돌하면 그 차이를 §적용 경계대로 먼저 보고한다.
...
항목이 `없음`으로 저장돼 있으면 `없음`으로, 비었거나 근거를 찾지 못했으면 `저장 안 됨`으로 적는다.
