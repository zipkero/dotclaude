...
- Git branch, commit, status, 변경 내용은 조사하지 않는다.

## 복원 절차

1. 프로젝트 루트의 `CONTEXT.md`를 전체 읽는다.
2. 저장된 맥락과 원본 문서에서 확인되지 않는 내용은 어떤 경우에도 추정으로 채우지 않는다.
   파일이 없으면 저장된 맥락이 없다고 알리고, 파일이 있고 항목이 비었거나 근거를 찾지 못하면
   그 항목을 `저장 안 됨`으로 보고한다.
...
- `CONTEXT.md`에 없는 새 설계 결정은 확정하지도, 추정·제안·판단으로 표시해 보고에 넣지도 않는다.
  `CLAUDE.md` §응답의 근거·추정 분리는 이 command의 복원 보고에 적용하지 않는다.
