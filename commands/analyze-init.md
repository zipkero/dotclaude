...
의무).
main은 반환 본문을 검토하고, 다음 유형에 걸리면서 선택이 실제로 갈리는 미해결 결정이 남아 있으면 기록 전에 질문으로 정리해(방식은 CLAUDE.md
§요청 해석) 그 결과를 §5 Decision Points의 채택 옵션으로 반영한 뒤 `features/<feature-dir>/analysis.md`에 기록하고, 아래 §덮어쓰기 규칙의 확인과
§README 갱신을 수행한다 — 공개 contract·외부 계약의 형태, 저장 형식·데이터 모델, 외부 연동의 의미, `SPEC §5.N`이 규정한 사용자 관찰 결과,
`SPEC §5.N` 충족 여부.
