...
4. 모호한 표현 (정의되지 않은 비유어, 소유 위치 불명)
5. 콩글리쉬·직역체: 도메인 외래어(`phase`, `trigger`, `Task`, `diff`, `evidence` 등)는 일관 차용을 허용하되, 영어 metaphor를 그대로 옮긴 표현(상류/하류/게이트/패스/풀 이름/단일 덩어리 류)은 자연스러운 한국어로 바꾼다. 메모리의 `feedback_no_literal_translation` 룰을 인용 기준으로 삼는다.
